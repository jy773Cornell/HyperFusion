...
Ur3ePoseResult ur3eGetTcpPose(const QString &serverUrl, QString *errorMessage)
{
    ...
    result.driverState = response.value(QStringLiteral("driver_state")).toString();
    if (response.value(QStringLiteral("tool0")).isArray())
    {
        result.hasTool0 = true;
        result.tool0 = poseFromJsonArray(response.value(QStringLiteral("tool0")).toArray());
    }
    const QJsonValue jointsVal = response.value(QStringLiteral("joints"));
    if (jointsVal.isObject())
    {
        const QJsonObject joints = jointsVal.toObject();
        result.jointNames = joints.value(QStringLiteral("names")).toVariant().toStringList();
        result.jointsRad =
            positionsFromJsonArray(joints.value(QStringLiteral("positions")).toArray());
    }
    return result;
}
