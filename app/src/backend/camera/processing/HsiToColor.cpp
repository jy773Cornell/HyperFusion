    ...
    QString *errorMessage)
{
    ...
    {
        if (errorMessage != nullptr)
            *errorMessage = QStringLiteral("Invalid image dimensions for RGB export.");
        return false;
    }

    const std::vector<double> truncatedWavelengths = truncateWavelengths(wavelengthsNm, truncateNm);
    const int bandCount = static_cast<int>(truncatedWavelengths.size());
    if (bandCount <= 1)
    {
        if (errorMessage != nullptr)
    ...
    const float normalization = static_cast<float>(1.0 / yIntegral);
    const int fullBands = static_cast<int>(wavelengthsNm.size());

    QImage image(samples, lines, QImage::Format_RGB888);
    if (image.isNull())
    {
        if (errorMessage != nullptr)
            *errorMessage = QStringLiteral("Could not allocate RGB image.");
    ...
    std::vector<float> fullSpectrum(static_cast<std::size_t>(fullBands));
    for (int line = 0; line < lines; ++line)
    {
        auto *scanLine = reinterpret_cast<std::uint8_t *>(image.scanLine(line));
        for (int sample = 0; sample < samples; ++sample)
        {
            provideReflectance(line, sample, fullSpectrum.data());

            float x = 0.0f;
            float y = 0.0f;
            float z = 0.0f;
            spectrumToXyz(integrationWeights, spectra, fullSpectrum.data(), x, y, z);
            x *= normalization;
            y *= normalization;
            z *= normalization;
    ...
    {
        if (errorMessage != nullptr)
            *errorMessage = QStringLiteral("Could not encode RGB PNG: %1").arg(outputPngPath);
        return false;
    }
    ...
    {
        if (errorMessage != nullptr)
            *errorMessage = QStringLiteral("Could not save RGB PNG: %1").arg(outputPngPath);
        return false;
    }
