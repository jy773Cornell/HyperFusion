            ...
            [this](const hf::processing::Gsam2ServerManager::State state, const QString &detail) {
                ...
                if (host_->captureRunGsamCheck_ != nullptr)
                {
                    if (state == hf::processing::Gsam2ServerManager::State::Running)
                    {
                        ...
                        host_->schedulePersistedUiSettingsSave();
                    }
                    else if (state == hf::processing::Gsam2ServerManager::State::Stopped
                             || state == hf::processing::Gsam2ServerManager::State::Failed)
                    {
                        host_->captureRunGsamCheck_->setChecked(false);
                    }
                }
                if (!detail.isEmpty())
...
void CapturePanelController::wireSettingsTabConnections()
{
    ...
    if (host_->captureGsamStartServerBtn_ != nullptr)
    {
        connect(host_->captureGsamStartServerBtn_,
                &QPushButton::clicked,
                this,
                [this]() {
                    if (gsam2ServerManager_ == nullptr)
                    ...
                    if (state == GsamState::Running || state == GsamState::Starting)
                        gsam2ServerManager_->stopServer();
                    else
...
QWidget *hf::capture::CapturePanelController::createStreamTab()
{
    ...
    for (std::size_t cameraIndex = 0; cameraIndex < 2; ++cameraIndex)
    {
        ...
        const QString cameraName =
            ui != nullptr ? host_->profileTabNameForUi(*ui) : defaultCameraTabName(cameraIndex);
...
void hf::capture::CapturePanelController::updatePositionControls(const StageState state)
{
    ...
    if (host_->capturePositionContent_ != nullptr)
        host_->capturePositionContent_->setEnabled(connected);
...
double hf::capture::CapturePanelController::autoRecordScanSpeedMmPerSec() const
{
    ...
    {
        const CameraSettings fx10eSettings = host_->buildCameraSettings(host_->camera1Ui_);
    ...
    const auto considerCamera = [&](const std::size_t cameraIndex) {
        ...
        const CameraSettings settings = host_->buildCameraSettings(ui);
        const double spatial = hf::effectiveSpatialMmPerPixelForStageCamera(
...
void hf::capture::CapturePanelController::updateDualCameraSyncControls()
{
    ...
    if (host_->camera2Ui_.frameRateSpin != nullptr)
    {
        ...
        host_->camera2Ui_.frameRateSpin->setEnabled(readyForCameraFeatures && !lockSwirFrameRate);
    }
}
...
void hf::capture::CapturePanelController::applyDualCameraScanSync(const bool applyToHardware)
{
    ...
    const hf::HardwareConfig &hw = hf::hardwareConfig();
    const CameraSettings fx10eSettings = host_->buildCameraSettings(host_->camera1Ui_);
    const CameraSettings swir3Settings = host_->buildCameraSettings(host_->camera2Ui_);
    ...
    if (applyToHardware && host_->coordinator_ != nullptr && host_->isCameraSessionActive(host_->camera2Ui_.state))
    {
        const CameraSettings settings = host_->buildCameraSettings(host_->camera2Ui_);
        host_->coordinator_->applySettings(host_->camera2Ui_.cameraIndex, settings);
    }
...
void hf::capture::CapturePanelController::updateRecorderControls()
{
    ...
    if (host_->captureUseStageForRecordingCheck_ != nullptr)
        host_->captureUseStageForRecordingCheck_->setEnabled(stageConnected && !scanActive);

    const bool preprocessingEnabled = useStage && !scanActive;
    if (host_->capturePreprocessingBox_ != nullptr)
        host_->capturePreprocessingBox_->setEnabled(preprocessingEnabled);
    ...
    const bool gsamChildEnabled = preprocessingEnabled && host_->capturePreprocessAfterScanCheck_ != nullptr
                                  && host_->capturePreprocessAfterScanCheck_->isChecked();
    if (host_->captureRunGsamCheck_ != nullptr)
        host_->captureRunGsamCheck_->setEnabled(gsamChildEnabled);
    if (host_->captureGsamPromptEdit_ != nullptr)
        host_->captureGsamPromptEdit_->setEnabled(gsamChildEnabled);
    if (host_->captureGsamSampleCountSpin_ != nullptr)
        host_->captureGsamSampleCountSpin_->setEnabled(gsamChildEnabled);
    if (host_->captureGsamStartServerBtn_ != nullptr)
        host_->captureGsamStartServerBtn_->setEnabled(preprocessingEnabled);
    ...
    if (host_->capturePositionContent_ != nullptr && stageConnected)
        host_->capturePositionContent_->setEnabled(!scanActive);

    host_->updateLightConnectionDisplay();
    host_->updateLightControlsEnabled();
    updateSessionUiLock();
...
{
    if (gsam2ServerManager_ == nullptr || host_->captureGsamStartServerBtn_ == nullptr)
        return;
    ...
    host_->captureGsamStartServerBtn_->setText(label);
    host_->captureGsamStartServerBtn_->setStyleSheet(
        QStringLiteral(
            "QPushButton {"
...
void hf::capture::CapturePanelController::updateSessionUiLock()
{
    ...
    if (host_->capturePositionBox_ != nullptr)
        host_->capturePositionBox_->setEnabled(!scanActive);
    ...
    if (host_->stageWorker_ != nullptr)
        updateStageConnectionControls(host_->stageWorker_->currentState(), false);

    updateCameraControls(host_->camera1Ui_, host_->camera1Ui_.state);
    updateCameraControls(host_->camera2Ui_, host_->camera2Ui_.state);
}
...
void hf::capture::CapturePanelController::beginCaptureRecordCompleteNotify()
{
    ...
    captureRecordCompleteHomingPending_ =
        !host_->performingGracefulShutdown_ && host_->stageWorker_ != nullptr
        && host_->stageWorker_->currentState() == StageState::Connected;

    tryNotifyCaptureRecordComplete();
}

void hf::capture::CapturePanelController::tryNotifyCaptureRecordComplete()
{
...
{
    if (host_->stageWorker_ == nullptr || host_->stageWorker_->currentState() != StageState::Connected)
    {
...
void hf::capture::CapturePanelController::beginCaptureMoveToFirstRefPosition()
{
    ...
    std::sort(selectedCameras.begin(),
              ...
              [this](const std::size_t a, const std::size_t b) {
                  const LumoCameraUi &uiA = a == 0 ? host_->camera1Ui_ : host_->camera2Ui_;
                  const LumoCameraUi &uiB = b == 0 ? host_->camera1Ui_ : host_->camera2Ui_;
...
void hf::capture::CapturePanelController::setSelectedCameraShutters(const bool open)
{
    ...
    {
        if (open)
            host_->coordinator_->openShutter(index);
        else
            host_->coordinator_->closeShutter(index);
    }
}
                                            ...
                                            const double speedMmPerSec)
{
    ...
    host_->stageWorker_->requestMoveAbsoluteMm(
        positionMm,
        moveSpeed,
        true,
...
void hf::capture::CapturePanelController::beginCaptureWhiteReferenceSequence()
{
    captureWhiteRefFramesCollected_ = {0, 0};
    captureWhiteRefWindowComplete_ = {false, false};
...
void hf::capture::CapturePanelController::updateWhiteReferenceScanGeometryForCurrentMode()
{
    std::vector<std::size_t> selectedCameras;
    if (!selectedCaptureCameraIndices(selectedCameras))
        return;
...
void hf::capture::CapturePanelController::beginCaptureWhiteReferenceScan()
{
    if (captureRecorderMode_ == CaptureRecorderMode::Preview)
        setSelectedCameraShutters(true);
...
void hf::capture::CapturePanelController::onCaptureWhiteReferenceSequenceComplete()
{
    ...
    if (host_->stageWorker_ != nullptr)
        host_->stageWorker_->requestStopMotion();
...
{
    if (captureRecorderMode_ == CaptureRecorderMode::Idle || host_->stageWorker_ == nullptr)
        return;

    host_->stageWorker_->requestPrimaryPosition([this](const double positionMm, const bool ok) {
        QMetaObject::invokeMethod(
            this,
            [this, positionMm, ok]() {
                if (captureRecorderMode_ == CaptureRecorderMode::Idle
...
{
    host_->stageWorker_->requestPrimaryPosition([this, distanceMm, speedMmPerSec, capturePhaseOnMoveStart](
        ...
        QMetaObject::invokeMethod(
            ...
            [this, positionMm, ok, distanceMm, speedMmPerSec, capturePhaseOnMoveStart]() {
                ...
                host_->stageWorker_->requestMoveRelativeMm(distanceMm, speedMmPerSec);
                if (capturePhaseOnMoveStart == CaptureScanPhase::SampleScan)
                {
...
void hf::capture::CapturePanelController::completeCaptureSequence()
{
    ...
    if (isStageRecordingEnabledInUi() && host_->stageWorker_ != nullptr)
        host_->stageWorker_->requestStopMotion();
...
void hf::capture::CapturePanelController::startCaptureSequence()
{
    ...
    if (host_->lighthouseWorker_ != nullptr && host_->isLighthouseSessionActive())
        host_->lighthouseWorker_->requestPollControllerPowerStatus();

    const LighthouseControllerPowerStatus powerStatus =
        host_->lighthouseWorker_ != nullptr ? host_->lighthouseWorker_->currentControllerPowerStatus()
...
{
    return host_->stageWorker_ != nullptr && host_->stageWorker_->currentState() == StageState::Connected;
}
...
QString hf::capture::CapturePanelController::captureCameraFolderName(const LumoCameraUi &ui) const
{
    ...
    QString slug = host_->profileTabNameForUi(ui).trimmed().toLower();
                                                 ...
                                                 QString &errorMessage) const
{
    ...
    for (const CaptureIlluminationMode illuminationMode : illuminationModes)
    {
        for (const std::size_t cameraIndex : cameraIndices)
        {
            ...
            stream.streamName = host_->profileTabNameForUi(ui);
            stream.settings = host_->buildCameraSettings(ui);
            stream.spectralBands = ui.spectralBands;
            stream.calibrationPackPath = host_->calibrationPackPath(ui);
...
void hf::capture::CapturePanelController::startRecord()
{
    ...
    if (!useStage)
    {
        ...
        {
            host_->appendLog(QStringLiteral("Capture record: cancelled by operator at hood preparation."));
            stopRecorder();
            return;
        }
...
void hf::capture::CapturePanelController::homeStageBeforeCapture()
{
    ...
    host_->stageHomingKind_ = StageHomingKind::BeforeCapture;
    host_->stageWorker_->requestHome();
}
...
void hf::capture::CapturePanelController::stopRecorder()
{
    ...
    if (host_->stageHomingKind_ == StageHomingKind::BeforeCapture)
        host_->stageHomingKind_ = StageHomingKind::None;
...
void hf::capture::CapturePanelController::homeStageAfterCapture()
{
    ...
    host_->stageHomingKind_ = StageHomingKind::AfterCapture;
    host_->stageWorker_->requestHome();
}
...
void hf::capture::CapturePanelController::updateCamerasList()
{
    ...
    const auto updateCheckbox = [this, &isConnected](LumoCameraUi &ui, QCheckBox *checkbox) {
        ...
        checkbox->setText(host_->profileTabNameForUi(ui));
        if (firstShow)
    ...
    for (const std::size_t cameraIndex : selected)
    {
        ...
        if (ui != nullptr)
        {
            host_->captureWaterfallPanes_[cameraIndex]->setTitle(
                host_->profileTabNameForUi(*ui) + QStringLiteral(" waterfall"));
        }
...
void hf::capture::CapturePanelController::updateCaptureCameraPositionRows()
{
    ...
    for (std::size_t cameraIndex = 0; cameraIndex < 2; ++cameraIndex)
    {
        ...
        const QString rowLabel = QStringLiteral("%1 position").arg(host_->profileTabNameForUi(*cameras[cameraIndex]));
