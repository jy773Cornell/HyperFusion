...
QWidget *MainWindow::createUr3eSettingsTab() {
  ...
  ur3eRobotIpEdit_->setPlaceholderText(
      QStringLiteral("Robot controller IP / hostname"));
  ...
  auto *valueHeader =
      new QLabel(QStringLiteral("Target (| = current)"), jointsBox);
  ...
  for (int jointIndex = 0; jointIndex < kUr3eJointCount; ++jointIndex) {
    auto *nameLabel =
        new QLabel(QString::fromUtf8(kUr3eJointNames[jointIndex]), jointsBox);
  ...
  ur3eSyncJointsBtn_ =
      new QPushButton(QStringLiteral("Sync Position"), jointsBox);
  ur3eStartRvizBtn_ = new QPushButton(QStringLiteral("Start RViz"), jointsBox);
  ur3eStartMoveItBtn_ =
      new QPushButton(QStringLiteral("Start MoveIt"), jointsBox);
  ...
  ur3eStartMoveItBtn_->setSizePolicy(QSizePolicy::Expanding,
                                     QSizePolicy::Fixed);
